...
import { selectCookieAuth } from "../global/globalSlice";
...
export const { updateEditTextAction, switchEditModeAction } = editSlice.actions;

// selectors
export const selectEditText = (state: RootState): string => state.edit.text;

export const selectEditMode = (state: RootState): EditMode => state.edit.mode;
...
export const saveEditToLocalStorage = (): AppThunk => (dispatch, getState) => {
    const state = getState();
    localStorage.setItem("editText", selectEditText(state));
    localStorage.setItem("editMode", selectEditMode(state));
};
