...
const insertAfterSelections = (
    ...
    const spec: TransactionSpec = view.state.changeByRange((range) => {
        const rangeObj = range.toJSON() as RangeObject;
        rangeObj.anchor =
            Math.max(rangeObj.anchor, rangeObj.head) +
            toInsert.length +
            selectOffset;
...
export const insertStrikethrough = (view: EditorView): void =>
    insertAroundSelections(view, "~~");

// insert the first line of a bulleted list
export const insertBulletedList = (view: EditorView): void =>
    insertAfterSelections(view, "\n- ");

// insert the first line of a numbered list
export const insertNumberedList = (view: EditorView): void =>
    insertAfterSelections(view, "\n1. ");

// insert the first line of a block quote
export const insertBlockQuote = (view: EditorView): void =>
    insertAfterSelections(view, "\n> ");
...
// execute an undo
export const undoEvent = (view: EditorView): boolean =>
    historyUndo({
        state: view.state,
...
// execute a redo
export const redoEvent = (view: EditorView): boolean =>
    historyRedo({
        state: view.state,
        dispatch: view.dispatch,
    });
...
export const clearAll = (view: EditorView): void =>
    view.dispatch({
        changes: {
            from: 0,
